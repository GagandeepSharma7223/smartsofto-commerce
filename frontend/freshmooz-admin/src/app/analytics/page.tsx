  ...
  return (
    <Shell title="Analytics">
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        <KpiCard label="Revenue (last 7 days)" value={kpi ? formatCurrency(kpi.last7) : 'INR --'} />
        <KpiCard label="Revenue (last 30 days)" value={kpi ? formatCurrency(kpi.last30) : 'INR --'} />
      </div>
        ...
        {monthly.length === 0 ? (
          ...
          <svg viewBox={`0 0 100 60`} className="w-full h-48">
            {monthly.map((p, idx) => {
              ...
              return (
                <g key={p.label}>
                  <rect x={x + 1} y={y} width={barW - 3} height={barH} fill="#6FAF3D" opacity="0.85" rx="1.5" />
...
function formatCurrency(n?: number) {
  if (typeof n !== 'number' || isNaN(n)) return 'INR --'
