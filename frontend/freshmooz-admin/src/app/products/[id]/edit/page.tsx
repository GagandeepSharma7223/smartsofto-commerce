...
export default function AdminEditProductPage({ params }: { params: { id: string } }) {
    ...
    const load = async () => {
      try {
        ...
        setForm({
          ...
          gstRate: String(p.gstRate ?? p.GstRate ?? 0),
  ...
  if (user === undefined) {
    return (
      <div className="landing"><main className="max-w-3xl mx-auto px-4 py-10"><h1 className="text-2xl font-bold mb-4">Admin - Edit Product</h1><LoadingState /></main></div>
    )
  }
    ...
    <div className="landing">
      <main className="max-w-3xl mx-auto px-4 py-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/products" className="text-[#2B7CBF]">Back</Link>
        </div>
        ...
        {form && (
          <form onSubmit={onSubmit} noValidate className="space-y-4">
            ...
            <button className="bg-[#6FAF3D] hover:bg-[#5F9B34] text-white px-4 py-2 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#4DB6E2]">Save Changes</button>
          </form>
        )}
      </main>
    </div>
