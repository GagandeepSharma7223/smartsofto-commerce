...
export default function CartIcon({ className = '', size = 'lg', showLabel = true }: Props) {
  ...
  return (
    <Link href="/cart" aria-label={`Cart`}
      ...
      <span className={`relative inline-flex items-center justify-center rounded-full border border-[#f0e6ee] hover:bg-[#f6eef4] ${dim}`}>
        ...
        {count > 0 && (
          <span className={`absolute -top-2 -right-2 bg-[#d9138a] text-white text-[10px] leading-none px-1.5 py-1 rounded-full font-bold shadow cart-badge ${bump ? 'bump' : ''}`}>
            {count}
          </span>
        )}
      </span>
