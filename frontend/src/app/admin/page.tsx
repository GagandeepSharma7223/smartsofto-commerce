...
export default function AdminDashboardPage() {
  ...
  const [summary, setSummary] = useState<ReturnType<typeof mapSummary> | null>(null)
    ...
    <div className="landing">
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {metricCard('Products', summary?.products ?? '—', 'Manage products →', '/admin/products')}
          {metricCard('Orders', summary?.orders ?? '—', 'Manage orders →', '/admin/orders')}
          {metricCard('Revenue (7d)', formatCurrency(summary?.revenue7d), 'View analytics →', '/admin/analytics')}
          {metricCard('Revenue (30d)', formatCurrency(summary?.revenue30d), 'View analytics →', '/admin/analytics')}
          {metricCard('Unpaid invoices', summary?.unpaid ?? '—', 'Invoices →', '/admin/invoices')}
          {metricCard('Partially paid', summary?.partial ?? '—', 'Invoices →', '/admin/invoices')}
        </div>
        ...
        {chartPoints.length > 0 && (
          <div className="mt-8 border rounded-xl bg-white shadow-sm p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/admin/analytics" className="text-[#d9138a] text-sm">Full analytics →</Link>
            </div>
...
function metricCard(title: string, value: string | number, cta: string, href: string) {
  return (
    <Link
      href={href}
      className="block border rounded-xl p-5 bg-white shadow-sm transition-transform transition-shadow duration-200 ease-out hover:-translate-y-1 hover:shadow-lg hover:border-[#ffd1e8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d9138a]">
      <div className="text-slate-600 text-sm">{title}</div>
      <div className="text-3xl font-extrabold">{value}</div>
      <div className="text-[#d9138a] mt-2">{cta}</div>
    </Link>
  )
}

function mapSummary(d: any) {
  return {
...
function MiniBarChart({ points }: { points: { label: string; total: number }[] }) {
    ...
    <div className="w-full">
      <svg viewBox={`0 0 100 ${height}`} className="w-full h-40">
        {points.map((p, idx) => {
          ...
          return (
            <g key={p.label}>
              <rect x={x + 1} y={y} width={barW - 3} height={barHeight} fill="#d9138a" opacity="0.8" rx="1.5" />
