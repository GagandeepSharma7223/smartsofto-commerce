    ...
    <div className="landing">
      <main className="max-w-3xl mx-auto px-4 pt-4 pb-8 grid md:grid-cols-2 gap-8">
          ...
          <form id="checkoutForm" onSubmit={placeOrder} className="space-y-5">
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input className={`w-full border rounded-md px-3 py-2 ${has('name') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} value={form.name} onChange={e => { setForm({ ...form, name: e.target.value }); mark('name', e.target.value.trim() ? undefined : 'Required') }} />
              </div>
              <div>
                <label className="block text-sm mb-1">Email</label>
                <input type="email" className={`w-full border rounded-md px-3 py-2 ${has('email') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} value={form.email} onChange={e => { setForm({ ...form, email: e.target.value }); mark('email', /.+@.+\..+/.test(e.target.value) ? undefined : 'Invalid email') }} />
              </div>
              <div>
                <label className="block text-sm mb-1">Phone</label>
                <input type="tel" placeholder="10-digit mobile" className={`w-full border rounded-md px-3 py-2 ${has('phone') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} value={form.phone} onChange={e => { setForm({ ...form, phone: e.target.value }); mark('phone', /^\d{10}$/.test(e.target.value.trim()) ? undefined : '10-digit number') }} />
              </div>
            </div>
            ...
            {savedAddresses.length > 0 && (
              <div className="space-y-2">
                ...
                {useSaved && (
                  <select
                    className="w-full border rounded-md px-3 py-2 border-slate-300"
                ...
                {useSaved && selectedAddress && (
                  <div className="flex items-center gap-3 text-sm">
                    <button type="button" className="text-[#d9138a] underline" onClick={()=>setEditing(selectedAddress)}>Edit</button>
            ...
            <div className="space-y-3">
              ...
              <input disabled={useSaved}
                className={`w-full border rounded-md px-3 py-2 ${useSaved ? 'bg-slate-100' : has('shipping.line1') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`}
              ...
              <input disabled={useSaved}
                className={`w-full border rounded-md px-3 py-2 ${useSaved ? 'bg-slate-100 border-slate-300' : 'border-slate-300'}`}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select disabled={useSaved} className={`w-full border rounded-md px-3 py-2 ${useSaved ? 'bg-slate-100 border-slate-300' : has('shipping.city') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} value={form.shipping.city} onChange={e=>{ setForm({...form, shipping: { ...form.shipping, city: e.target.value }}); mark('shipping.city', /^gurugram$/i.test(e.target.value) ? undefined : 'Only Gurugram') }}>
                    <option value="Gurugram">Gurugram</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs mb-1">State</label>
                  <input disabled={useSaved} className={`w-full border rounded-md px-3 py-2 ${useSaved ? 'bg-slate-100 border-slate-300' : has('shipping.state') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} value={form.shipping.state} onChange={e=>{ setForm({...form, shipping: { ...form.shipping, state: e.target.value }}); mark('shipping.state', e.target.value.trim().toLowerCase()==='haryana' ? undefined : 'Must be Haryana') }} />
                </div>
                <div>
                  <label className="block text-xs mb-1">Pincode</label>
                  <input disabled={useSaved} className={`w-full border rounded-md px-3 py-2 ${useSaved ? 'bg-slate-100 border-slate-300' : has('shipping.pincode') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} placeholder="6-digit pincode" value={form.shipping.pincode} onChange={e=>{ const v = e.target.value; setForm({...form, shipping: { ...form.shipping, pincode: v }}); mark('shipping.pincode', /^\d{6}$/.test(v.replace(/[\s-]/g, '').trim()) ? undefined : 'Enter 6-digit pincode') }} />
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              {!form.billingSameAsShipping && (
                <div className="space-y-3">
                  ...
                  <input className={`w-full border rounded-md px-3 py-2 ${has('billing.line1') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} placeholder="Address line 1" value={form.billing.line1} onChange={e=>{ setForm({...form, billing: { ...form.billing, line1: e.target.value }}); mark('billing.line1', e.target.value.trim() ? undefined : 'Required') }} />
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-xs mb-1">City</label>
                      <select className={`w-full border rounded-md px-3 py-2 ${has('billing.city') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} value={form.billing.city} onChange={e=>{ setForm({...form, billing: { ...form.billing, city: e.target.value }}); mark('billing.city', /^gurugram$/i.test(e.target.value) ? undefined : 'Only Gurugram') }}>
                        <option value="Gurugram">Gurugram</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs mb-1">State</label>
                      <input className={`w-full border rounded-md px-3 py-2 ${has('billing.state') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} value={form.billing.state} onChange={e=>{ setForm({...form, billing: { ...form.billing, state: e.target.value }}); mark('billing.state', e.target.value.trim().toLowerCase()==='haryana' ? undefined : 'Must be Haryana') }} />
                    </div>
                    <div>
                      <label className="block text-xs mb-1">Pincode</label>
                      <input className={`w-full border rounded-md px-3 py-2 ${has('billing.pincode') ? 'border-red-500 focus:ring-2 focus:ring-red-500' : 'border-slate-300'}`} placeholder="6-digit pincode" value={form.billing.pincode} onChange={e=>{ const v = e.target.value; setForm({...form, billing: { ...form.billing, pincode: v }}); mark('billing.pincode', /^\d{6}$/.test(v.replace(/[\s-]/g, '').trim()) ? undefined : 'Enter 6-digit pincode') }} />
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="md:col-span-2 mt-6">
          <button type="submit" form="checkoutForm" disabled={loading} className="bg-[#d9138a] hover:bg-[#c2107c] text-white px-4 py-2 rounded-md w-full md:w-auto">{loading ? 'Placing…' : 'Place order'}</button>
        </div>
      </main>
    </div>
  )
...
function AddAddressInline({ onAdded }: { onAdded: (addr: ClientAddress) => void }) {
  ...
  const onSave = async () => {
    ...
    setSaving(true)
    try{
      const payload = { label, addressLine1: line1, addressLine2: line2, city: 'Gurugram', state, postalCode: pincode, phone, country: 'India' }
  ...
  if (!open) return <button type="button" className="text-[#d9138a] underline text-sm" onClick={()=>setOpen(true)}>+ Add new address</button>
  return (
    <div className="border rounded-md p-3 space-y-2">
      <div className="grid md:grid-cols-2 gap-2">
        <input className="border rounded-md px-3 py-2" placeholder="Label (e.g., Home, Office)" value={label} onChange={e=>setLabel(e.target.value)} />
        <input className="border rounded-md px-3 py-2" placeholder="Phone (optional)" value={phone} onChange={e=>setPhone(e.target.value)} />
      </div>
      <input className="border rounded-md px-3 py-2" placeholder="Address line 1" value={line1} onChange={e=>setLine1(e.target.value)} />
      <input className="border rounded-md px-3 py-2" placeholder="Address line 2 (optional)" value={line2} onChange={e=>setLine2(e.target.value)} />
      <div className="grid grid-cols-3 gap-2">
        <input className="border rounded-md px-3 py-2 bg-slate-100" value={'Gurugram'} disabled />
        <input className="border rounded-md px-3 py-2" value={state} onChange={e=>setState(e.target.value)} />
        <input className="border rounded-md px-3 py-2" placeholder="6-digit pincode" value={pincode} onChange={e=>setPincode(e.target.value)} />
      </div>
      {err && <div className="text-red-600 text-sm">{err}</div>}
      <div className="flex items-center gap-2">
        <button type="button" className="bg-[#d9138a] text-white px-3 py-1 rounded" disabled={saving} onClick={onSave}>{saving?'Saving...':'Save address'}</button>
...
function EditAddressInline({ address, onSaved, onCancel }: { address: ClientAddress, onSaved: (addr: ClientAddress)=>void, onCancel: ()=>void }) {
  ...
  return (
    <div className="border rounded-md p-3 space-y-2 mt-2">
      <div className="grid md:grid-cols-2 gap-2">
        <input className="border rounded-md px-3 py-2" placeholder="Label" value={label} onChange={e=>setLabel(e.target.value)} />
        <input className="border rounded-md px-3 py-2" placeholder="Phone (optional)" value={phone} onChange={e=>setPhone(e.target.value)} />
      </div>
      <input className="border rounded-md px-3 py-2" placeholder="Address line 1" value={line1} onChange={e=>setLine1(e.target.value)} />
      <input className="border rounded-md px-3 py-2" placeholder="Address line 2 (optional)" value={line2} onChange={e=>setLine2(e.target.value)} />
      ...
      {err && <div className="text-red-600 text-sm">{err}</div>}
      <div className="flex items-center gap-2">
        <button type="button" className="bg-[#d9138a] text-white px-3 py-1 rounded" disabled={saving} onClick={onSave}>{saving?'Saving...':'Save changes'}</button>
